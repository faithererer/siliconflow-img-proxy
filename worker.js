 ...
 * - Fixed: OpenAI-style image input mapping for Image-Edit models.
 *
 * Endpoints:
 ...
 *
 * Auth:
 * - Proxy access keys: ALLOW_CLIENT_KEY / ALLOW_CLIENT_KEYS 	(independent from SILICONFLOW_API_KEY)
 * - Upstream siliconflow key: SILICONFLOW_API_KEY (server-only; client cannot override)
 */
...
async function imagesGenerations(openaiBody, req, env) {
	...
	// Map size -> image_size (edit 系列通常不支持 image_size，由上游自行忽略也无妨)
	const imageSize = openaiBody?.sf_image_size || openaiBody?.size;
	if (typeof imageSize === "string") commonSfPayload.image_size = imageSize;

	// Map OpenAI-style image input to SF `image` (for edit models, required)
	const isEdit = isImageEditModel(model);
	const img = pickImageFromBody(openaiBody); // data:URI 或 http(s) URL
	if (isEdit) {
		if (!img) {
			return json(
		...
		// 对非编辑模型：如果用户传了 image，也顺带透传（有的模型会用作参考）
		if (img) commonSfPayload.image = img;
	}

	// Extra knobs passthrough
	...
	// 如果提供 batch_size 且 n==1，就一次批量（模型需支持）；否则按 n 次循环
...
async function chatCompletions(openaiBody, req, env) {
	...
	// 尝试从“最新 user 消息”的 content[] 提取 OpenAI 风格图片
	let img = pickImageFromMessage(lastUser);
	// 没有的话也支持顶层 image / image_url / images（有人会直接放在 chat body 顶层）
	...
	if (isEdit) {
		if (!img) {
			return json(
				{ error: { message: "image is required for image-edit models in chat (provide messages[].content[].image_url or a data:image/...)", type: "invalid_request_error" } },
				400
			);
		}
		commonSfPayload.image = img;
...
/** 是否编辑模型（当前按名称包含 "Qwen/Qwen-Image-Edit" 来判断） */
function isImageEditModel(model) {
	return /Qwen\/Qwen-Image-Edit/i.test(model);
}

/** 从顶层 JSON 里提 OpenAI 风格的 image/image_url/images */
...
/** 从消息（OpenAI chat 格式）取图片: content: [{type:"image_url", image_url:{url}}] | Markdown | data:URI */
function pickImageFromMessage(msg) {
	if (!msg) return null;

	// content 为数组（新版 OpenAI）
	if (Array.isArray(msg.content)) {
		for (const it of msg.content) {
			// 标准：{type:"image_url", image_url:{url}}
			if (it && it.type === "image_url") {
				const u = it.image_url?.url || it.image_url; // 宽松
				const s = coerceImageStringOrObj(u);
				if (s) return s;
			}
			// 一些 SDK：{type:"input_image", image_url:{url}}
	...
	// content 为字符串：尝试从 Markdown 或裸 URL / data:URI 抽取
...
/** 接受 string 或 {url:string}，转成 string（data:URI 或 http(s)） */
